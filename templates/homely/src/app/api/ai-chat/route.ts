  ...
  try {
    ...
    if (
      ...
      if (message.includes("villa")) {
        const villaType = await PropertyType.findOne({ name: /villa/i });
        if (villaType) searchQuery.propertyType = villaType._id;
      } else if (message.includes("apartment")) {
        const aptType = await PropertyType.findOne({ name: /apartment/i });
        if (aptType) searchQuery.propertyType = aptType._id;
      } else if (message.includes("office")) {
        const officeType = await PropertyType.findOne({ name: /office/i });
      ...
      properties = await Property.find(searchQuery)
        ...
        .select(
          "title location price bedrooms bathrooms area propertyType images"
        )
        .limit(6)
        .lean();

      // If no specific matches, get featured or recent properties

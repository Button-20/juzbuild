...
async function getBlogBySlug(slug: string): Promise<Blog | null> {
  try {
    // Direct database call instead of HTTP fetch for server-side rendering
    await connectDB();
...
async function getRelatedBlogs(
  ...
  try {
    ...
    // If tags are provided, find blogs with similar tags
    if (tags.length > 0) {
      query.tags = { $in: tags };
    }
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 sm:gap-6 mt-8 sm:mt-10 lg:mt-12">
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 lg:gap-7">
                <div className="flex items-center gap-2 sm:gap-3">
                  <Icon icon={"ph:clock"} width={16} height={16} className="sm:w-5 sm:h-5" />
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <Icon icon="solar:eye-outline" className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  <span className="text-xs sm:text-sm text-dark dark:text-white">
                    {blog.views} views
                  </span>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <Icon icon="solar:clock-circle-outline" className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  <span className="text-xs sm:text-sm text-dark dark:text-white">
                    {blog.readTime} min
                  </span>
                </div>

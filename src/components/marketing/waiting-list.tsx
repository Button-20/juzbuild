...
const WaitingList = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(
          "🎉 Thanks for signing up! You'll be among the first to know when we launch."
        );
        setEmail("");
      } else {
        ...
        <div className="flex flex-col items-center justify-center text-center gap-y-3 md:gap-y-4 bg-background/0">
          <Container className="relative">
            ...
            <p className="text-sm md:text-md mb-6 md:mb-8 text-muted-foreground max-w-lg mx-auto px-4">
              We're building the future of AI-powered real estate platforms. Get
              notified when we launch and be among the first to experience the
              revolution.
            </p>
